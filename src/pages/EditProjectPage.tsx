...
import { Plus, X, ArrowRight, AlertCircle, Trash2, Check } from 'lucide-react';
import { useNav } from '@/nav';
import { useAuth } from '@/lib/auth';
import { PageContainer, PageHeader } from '@/components/Layout';
import { fetchAllSkills, fetchProjectById, updateProject, PROJECT_CATEGORIES, PROJECT_STATUSES, PROFICIENCY_LABELS, IMPORTANCE_LEVELS } from '@/lib/db';
import type { ProjectCategory, ProjectStatus, Role, ProjectRequirement } from '@/types';
...
export function EditProjectPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [skillsData, projectData] = await Promise.all([fetchAllSkills(), fetchProjectById(id)]);
        setAllSkills(skillsData.map((s) => ({ id: s.id, name: s.name })));
        if (!projectData) {
          setError('Project not found');
          return;
        }
        if (projectData.project.ownerId !== user?.id) {
          setError('You can only edit projects you own');
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate({ name: 'project', id }), 800);
    } catch (err) {
    ...
    return (
      <PageContainer>
        <div className="card p-12 text-center max-w-md mx-auto mt-12">
          ...
          <h2 className="font-display font-700 text-2xl text-ink-900">Project updated!</h2>
          <p className="text-sm text-ink-500 mt-2">Redirecting to project page…</p>
        </div>
      </PageContainer>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="card p-6 space-y-5">
          ...
          <div>
            ...
            <input value={tagline} onChange={(e) => setTagline(e.target.value)} className="input" required />
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as ProjectStatus)} className="input appearance-none">
        ...
        <div className="card p-6">
          ...
          <input value={skillSearch} onChange={(e) => setSkillSearch(e.target.value)} placeholder="Search skills to add..." className="input mb-3" />
              ...
              {skillRequirements.map((req) => (
                <div key={req.skillId} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl bg-ink-50 border border-ink-100">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <select value={req.requiredProficiency} onChange={(e) => updateRequirement(req.skillId, 'requiredProficiency', Number(e.target.value))} className="text-xs rounded-lg border border-ink-200 px-2 py-1.5 bg-white">
                      {Object.entries(PROFICIENCY_LABELS).map(([level, label]) => <option key={level} value={level}>{label}</option>)}
                    </select>
                    <select value={req.importance} onChange={(e) => updateRequirement(req.skillId, 'importance', e.target.value)} className="text-xs rounded-lg border border-ink-200 px-2 py-1.5 bg-white">
                      {IMPORTANCE_LEVELS.map((imp) => <option key={imp} value={imp}>{imp}</option>)}
                    </select>
                    <div className="flex items-center gap-1.5">
                      <label className="text-xs text-ink-500">People</label>
                      <input type="number" min={1} max={10} value={req.peopleNeeded} onChange={(e) => updateRequirement(req.skillId, 'peopleNeeded', Math.max(1, Number(e.target.value)))} className="w-14 text-xs rounded-lg border border-ink-200 px-2 py-1.5 bg-white text-center" />
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="flex gap-3">
          <button type="button" onClick={() => navigate({ name: 'project', id })} className="btn-secondary flex-1">Cancel</button>
